#!/bin/bash

# Clear all functions
unset -f wpuplugincreator_add_dependency;
unset -f wpuplugincreator_create_github_actions;
unset -f wpuplugincreator_create_inc;
unset -f wpuplugincreator_find_folder;
unset -f wpuplugincreator_get_github_repo_url;
unset -f wpuplugincreator_get_plugin_id;
unset -f wpuplugincreator_has_github_actions;
unset -f wpuplugincreator_install_wpcli;
unset -f wpuplugincreator_migrate_from_master_to_main;
unset -f wpuplugincreator_plugin_id;
unset -f wpuplugincreator_plugin_file;
unset -f wpuplugincreator_protect_dir;
unset -f wpuplugincreator_regenerate_languages;
unset -f wpuplugincreator_remove_markers;
unset -f wpuplugincreator_replace_namespace;
unset -f wpuplugincreator_self_update_dependencies;
unset -f wpuplugincreator_set_values;
unset -f wpuplugincreator_update_add_abspath_protection;
unset -f wpuplugincreator_update_check_code;
unset -f wpuplugincreator_update_check_unescaped_translations;
unset -f wpuplugincreator_update_dependency;
unset -f wpuplugincreator_update_check_github_actions;
unset -f wpuplugincreator_update_github_actions;
unset -f wpuplugincreator_update_github_actions_file;
unset -f wpuplugincreator_update_gitignore;
unset -f wpuplugincreator_update_license;
unset -f wpuplugincreator_update_main_file_version_replace;
unset -f wpuplugincreator_update_main_file;
unset -f wpuplugincreator_update_protect;
unset -f wpuplugincreator_update_uninstall;
unset -f wpuplugincreator_update_translations;
unset -f wpuplugincreator_upgrade_wpubaseplugin;
unset -f wpuplugincreator_wpcli_command;

# Clear variables
unset -v wpuplugincreator__translation_string;
unset -v wpuplugincreator__translation_file;
unset -v wpuplugincreator__translation_dir;
